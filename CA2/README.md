...
2. Generate a new Maven project using the Spring Initializr and select the same dependencies as in the Gradle project.
3. Copy the contents of the extracted Spring Initializr zip file to the Part2_Maven folder.
4. Copy the src folder from the basic module of CA1 to CA2/Part2_Maven.
5. Copy the webpack.config.js and package.json files from the basic module of CA1 to CA2/Part2_Maven.
6. Update the Employee class to change all javax.persistence imports to jakarta.persistence.
7. Add the following dependencies to the pom.xml file:
   ...
   ```
8. Add the frontend-maven-plugin to the pom.xml file to manage the frontend:
   ```xml
    <plugin>
         ...
         <configuration>
              ...
              <installScript>install</installScript>
              <runScript>build</runScript>
              <cleanScript>clean</cleanScript>
    ...
    ```
9. Add the following plugin to copy the generated jar to a folder named "dist":
    ```xml
    <plugin>
    ...
    ```
10. Add the following plugin to delete the files generated by webpack before the clean task:
   ```xml
    <plugin>
...
14. Test if the jar file is copied to the "dist" folder (in this alternative solution, the task is done automatically when the project is built):
    ```bash
    ls dist
    ```
If the jar file is present, the task was executed successfully.

15. Test if the files generated by webpack are deleted before the clean task is executed:
      ```bash
      ./mvnw clean
...
16. Commit the changes and link them to the corresponding GitHub issue:
    ```bash
    git add .
